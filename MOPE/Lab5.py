...
matrix_with_min_max_x = np.array([[-3, 7], [0, 9], [-8, 10]])
m = 3
...
def cochran_check(Y_matrix_, N):
    ...
    return Gp < Criteries.Criteries.get_cohren_value(N, m-1, 0.05)
...
def students_t_test(norm_matrix_, Y_matrix_, N):
    ...
    return np.where(t > Criteries.Criteries.get_student_value((m-1)*N, 0.05))
...
def phisher_criterion(Y_matrix, d, N):
    ...
    return Fp < Criteries.Criteries.get_fisher_value((m-1)*N, N-d, 0.05)
...
print("Кількість переходів до рівняння з ефектом взаємодії:", count_1_state)
print("Кількість переходів до рівняння з квадратичними членами:", count_2_state)
