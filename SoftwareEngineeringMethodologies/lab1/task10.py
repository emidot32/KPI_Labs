...
def get_conforming_seas(ships, hor_space_nums, ver_space_nums):
    """Gets all seas conforming sea battle conditions"""
    seas = []
    all_absent_ships = []
    all_free_1 = []
    hor_rows = [get_variants(v, ships) for v in hor_space_nums.values()]
    ver_rows = [get_variants(v, ships) for v in ver_space_nums.values()]
    for cross in itertools.product(*hor_rows, *ver_rows):
        is_appropriate = check_rows_columns(
        ...
        if is_appropriate:
            ...
            check, absent_ships, free_1 = check_sea(
                sea, list(hor_space_nums.keys()), list(ver_space_nums.keys()), ships)
            if sea not in seas and check:
                seas.append(sea)
                all_absent_ships.append(absent_ships)
                all_free_1.append(free_1)
    return seas, all_absent_ships, all_free_1


def check_rows_columns(cross, hor_space_keys, ver_space_keys):
...
if __name__ == '__main__':
    # horizontal_space_numbers = {1: [2, 1, 2], 4: [1, 4, 1], 7: [1, 1, 1]}
    # vertical_space_numbers = {2: [1, 2, 4], 5: [1, 2, 3], 7: [3, 1, 1]}
    horizontal_space_numbers = {1: [3, 1, 1], 4: [5, 2], 7: [1, 2, 1]}
    vertical_space_numbers = {2: [5, 1, 1], 5: [1, 2, 4], 7: [4, 1]}
    flotilla = [4, 3, 3, 2, 2, 2, 1, 1, 1, 1]
    all_seas, all_absent_ships_list, all_free_ones = \
        get_conforming_seas(flotilla, horizontal_space_numbers, vertical_space_numbers)
    for index, sea_ in enumerate(all_seas):
        for extended_sea in \
                add_absent_ships(sea_, all_absent_ships_list[index], all_free_ones[index]):
            print_sea(extended_sea)
